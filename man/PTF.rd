...
The techniques are implemented in a consistent way and include functions for fitting a model to a training set of species and environmental data, with the function name named after the technique: that is, \code{WA} fits a weighted averaging model. Any model can be cross-validated using the \code{crossval} function, which allows internal cross-validation using leave-one-out, leave-n-out, or bootstrapping. There are a number of generic functions that can be used to summarise and diagnose the models: (\code{print}, \code{summary}, \code{performance} and \code{plot}. Some techniques have additional diagnostic functions such as \code{screeplot} and \code{rand.t.test} to help estimate the approproate number of components (WAPLS), factors (IKFA) or number of analogues (IKFA).
