WA <- function(y, x, tolDW = FALSE, use.N2=TRUE, check.data=TRUE, lean=FALSE)
{
  y <- as.matrix(y)
  if (!is.null(dim(x)))
     x <- x[, 1]
  ...
  model <- WA.fit(y, x, tolDW, use.N2=use.N2, lean=lean)
  xHat <- predict.internal.WA(model, y, lean=lean)
  call.fit <- as.call(list(quote(WA.fit), y=quote(y), x=quote(x), tolDW=tolDW, use.N2=use.N2, lean=TRUE))
...
WA.fit <- function(y, x, tolDW=FALSE, use.N2=TRUE, lean=FALSE) {
  if (is.data.frame(y))
     y <- as.matrix(y)
...
#    tol[tol < tol.mean/10] <- tol.mean
    tol[is.na(tol) | tol < 0.01] <- tol.mean
    tol2 <- tol^2
    rS.t <- rowSums(sweep(y, 2, tol2, "/"), na.rm=TRUE)
    u.tol <- u2/tol2
    u.tol[bad] <- 1E8
    xHat.t <- y %*% u.tol / rS.t
    xHat.t[is.nan(xHat)] <- NA
  ...
  result <- list(coefficients=coef, deshrink.coefficients=deshrink.coefficients, tolDW=tolDW)
  result
}
...
predict.internal.WA <- function(object, y, lean=FALSE, ...) {
  ...
  if (object$tolDW) {
    ...
    xHat.t <- cbind(xHat.inv.t, xHat.cla.t)
    if (!lean)
    	colnames(xHat.t) <- c("WA.inv.tol", "WA.cla.tol")
    xHat <- cbind(xHat, xHat.t)
  }
	xHat
}
...
{ 
  ...
  if (!object$tolDW)
     stop("Model does not contain tolerance downweighting - nothing to test.")
  npls <- nrow(p$crossval)
  delta1 <- (p$crossval[3, 1] - p$crossval[1, 1]) / p$crossval[1, 1] * 100
  delta2 <- (p$crossval[4, 1] - p$crossval[2, 1]) / p$crossval[2, 1] * 100
